...
class LongTermMemory(Memory):
    """Long-term — SQLite, persists across sessions."""
    ...
    def _init_db(self) -> None:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_imp  ON memories(importance DESC)")
        conn.commit()
        conn.close()
    ...
    async def query(self, query: str = "", memory_type: Optional[str] = None, limit: int = 10) -> List[MemoryContent]:
        conn   = sqlite3.connect(self.db_path)
        sql    = "SELECT content, mime_type, metadata FROM memories WHERE " + ("content LIKE ?" if query else "1=1")
        ...
        conn.close()
        return [MemoryContent(content=r[0], mime_type=r[1], metadata=json.loads(r[2]) if r[2] else None) for r in rows]
