...
async def execute_sql(
    db_path: Annotated[str, "Database path"],
...
async def csv_to_db(
    csv_path: Annotated[str, "CSV file path"],
    db_path: Annotated[str, "Database path"],
...
# ✅ FIXED: Added descriptions (MANDATORY)
sql_tool = FunctionTool(
    execute_sql,
    description="Execute SQL queries like SELECT, INSERT, UPDATE safely"
)

csv_tool = FunctionTool(
    csv_to_db,
    description="Convert CSV file into SQLite database table"
)
...
db_agent = AssistantAgent(
    ...
    system_message="You handle databases safely using SQL."
)